...
// before each scenario file by lexiray-ops, then executed as one script:
...
let fixtureDirectoryURL = URL(fileURLWithPath: repoRoot).appendingPathComponent("Tools/LexiRayGUI/fixtures", isDirectory: true)
...
final class ControlledPanelBackdrop {
  ...
  func waitUntilVerified(
    targetWindowID: UInt32,
    processIdentifier: pid_t,
    timeout: TimeInterval = 0.75
  ) -> String? {
    let deadline = Date().addingTimeInterval(timeout)
    var lastError = "controlled panel backdrop did not reach a verified window-server state"
    repeat {
      panel.order(.below, relativeTo: Int(targetWindowID))
  ...
  func close() {
    let windowNumber = UInt32(panel.windowNumber)
    panel.orderOut(nil)
    panel.close()
    let deadline = Date().addingTimeInterval(1)
    while Date() < deadline,
          allWindows().contains(where: { windowID($0) == windowNumber && windowOwnerPID($0) == getpid() })
    {
      RunLoop.current.run(until: Date().addingTimeInterval(0.05))
    }
  }
...
func captureOwnedWindow(
  ...
  if isFloatingPanelWindow(window) {
    ...
    guard controlledBackdrop?.waitUntilVerified(
      targetWindowID: id,
      processIdentifier: processIdentifier
    ) == nil
    else {
...
func snapPanelWithOpenLanguageMenu(
  ...
  guard backdrop.waitUntilVerified(
    targetWindowID: targetWindowID,
    processIdentifier: processIdentifier
  ) == nil
  else {
...
func ensureAppRunning() {
  ...
  let launchIdentityMatched = waitFor("launched LexiRay process identity", timeout: 3) {
    let matches = workspacePathInstances().filter {
      applicationMatchesWorkspaceProcess($0, expectedArguments: launchArguments)
    }
    guard matches.count == 1 else {
      return false
    }
    launchedApplication = matches[0]
    return true
  }
  guard launchIdentityMatched, let launchedApplication else {
    let reportedPID = launchedProcessIdentifier.map(String.init) ?? "unavailable"
    blocked(
      "launched LexiRay PID \(reportedPID) did not resolve to exactly one workspace executable "
        + "and acceptance arguments; it was not recorded or terminated"
    )
  }
  let resolvedProcessIdentifier = launchedApplication.processIdentifier
  workspaceProcessIdentifier = resolvedProcessIdentifier
